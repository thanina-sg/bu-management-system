...
export function BooksPanel({ role }: { role: LoggedInRole }) {
  ...
  const [editFields, setEditFields] = useState<{ status: BookStatus; location: string; category: Book["category"] }>({ status: "Available", location: "", category: "Computer Science" });
  const [toast, setToast] = useState<string | null>(null);

  const showToast = (msg: string) => { setToast(msg); setTimeout(() => setToast(null), 3000); };
  const startEdit = (b: Book) => { setEditingId(b.id); setEditFields({ status: b.status, location: b.location, category: b.category }); };
  const cancelEdit = () => setEditingId(null);

  const saveEdit = (id: string) => {
    setBooks((prev) => prev.map((b) => b.id === id ? { ...b, ...editFields } : b));
    const book = books.find((b) => b.id === id);
    setEditingId(null);
    showToast(`"${book?.title}" updated.`);
  };
  ...
  const handleAddBook = (book: Book) => {
    ...
    showToast(`"${book.title}" added to catalog.`);
  };
                ...
                <tr key={b.id} className="border-b border-ink-100 last:border-b-0">
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0"><div className="max-w-[180px] truncate text-xs font-semibold text-ink-900">{b.title}</div><div className="text-[10px] text-ink-500">{b.author}</div></div>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-xs text-ink-700">{b.isbn}</td>
                  <td className="px-5 py-3">
                    {editingId === b.id
                      ? <select value={editFields.category} onChange={(e) => setEditFields((f) => ({ ...f, category: e.target.value as Book["category"] }))} className="rounded border border-ink-100 bg-white px-2 py-1 text-xs text-ink-900 outline-none focus:border-brand-500"><option value="Computer Science">Computer Science</option><option value="Software Engineering">Software Engineering</option></select>
                      : <span className="text-xs text-ink-700">{b.category}</span>}
                  </td>
                  ...
                  <td className="px-5 py-3">
                    {editingId === b.id
                      ? <select value={editFields.status} onChange={(e) => setEditFields((f) => ({ ...f, status: e.target.value as BookStatus }))} className="rounded border border-ink-100 bg-white px-2 py-1 text-xs text-ink-900 outline-none focus:border-brand-500"><option value="Available">Available</option><option value="Borrowed">Borrowed</option></select>
                      : <BookStatusBadge status={b.status} />}
                  </td>
                  <td className="px-5 py-3">
                    {editingId === b.id ? (
                      <div className="flex gap-2">
                        <button onClick={() => saveEdit(b.id)} className="rounded bg-brand-700 px-3 py-1 text-[10px] font-semibold text-white hover:bg-brand-600">Save</button>
                      ...
                      <div className="flex gap-2">
                        ...
                        {role === "Admin" && (
                          <button onClick={() => deleteBook(b.id)} className="rounded border border-rose-200 px-3 py-1 text-[10px] font-semibold text-rose-600 hover:bg-rose-50">Delete</button>
                        )}
                      </div>
                    )}
                  </td>
            ...
            <div key={b.id} className="rounded-lg border border-ink-100 bg-surface-50 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0"><div className="truncate text-xs font-semibold text-ink-900">{b.title}</div><div className="text-[10px] text-ink-500">{b.author}</div></div>
              ...
              <div className="mt-3">
                {editingId === b.id ? (
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => startEdit(b)} className="rounded border border-ink-100 px-3 py-1 text-[10px] font-semibold text-ink-700 hover:bg-surface-50">Edit</button>
                    {role === "Admin" && <button onClick={() => deleteBook(b.id)} className="rounded border border-rose-200 px-3 py-1 text-[10px] font-semibold text-rose-600 hover:bg-rose-50">Delete</button>}
                  </div>
                )}
              </div>
            </div>
