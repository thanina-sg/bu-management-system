    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      {isLoading && (
        <div className="mt-6 text-center">
          <div className="text-[10px] text-ink-500">Loading book details...</div>
        </div>
      )}
      ...
      {!isLoading && !error && book && (
        <>
          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[236px_1fr]">
            <div>
              ...
              <div className="mt-3 rounded-lg border border-ink-100 bg-white p-4 shadow-soft">
                ...
                >
                  {book.status || book.disponible === false ? "Borrowed" : "Available"}
                </div>
                {book.status === "Borrowed" || !book.disponible ? (
                  <button
                    type="button"
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                {recommended.length > 0 ? (
                  ...
                  <div className="col-span-3 py-6 text-center">
                    <p className="text-[10px] text-ink-500">No recommendations available</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {showReservation && (
            <ReservationModal
              bookTitle={book.title || book.titre || ''}
              isbn={book.isbn}
